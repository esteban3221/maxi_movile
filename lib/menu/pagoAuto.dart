...
class PagoAutoPage extends StatefulWidget {
  const PagoAutoPage({super.key});

  @override
  State<PagoAutoPage> createState() => _PagoAutoPageState();
}

class _PagoAutoPageState extends State<PagoAutoPage> {
  ...
  void _enviarPago(BuildContext context) async {
    ...
    final request = http.post(
      Uri.parse('${GlobalVar().apiUrl}accion/inicia_pago'),
    ...
    AsyncHttpService.consumeAndDo<Map<String, dynamic>>(
      ...
      onSuccess: (Map<String, dynamic> data) {
        setState(() {
          _mensaje = '✅ Pago registrada con éxito! ID: ${data['id'] ?? 'N/A'}';
        });
